...
selectQuery = {...selectQuery, password:0, is_global_password:0 }
    let term;
    // if (search) {
...
fileController.AddFolders = async (req, res, next) => {
  try {
    const data = req.body;
    const { is_password_protected=false, set_password: password, is_global_password=false } = data;
    const parent_id = req.params.id;
    if (is_password_protected === true && !is_global_password && !password?.length) {
      return otherHelper.sendResponse(res, httpStatus.BAD_REQUEST, false, null, null, 'Password is required', null);
    }
    let hashPwd= '';
    let salt = await bcrypt.genSalt(10);
     hashPwd = is_password_protected && !is_global_password && await bcrypt.hash(password, salt) || hashPwd;
     if(is_password_protected && is_global_password){
       const userPassword = await userSch.findById(req.user.id).select({ password: 1 });
       hashPwd = userPassword.password;
     }
